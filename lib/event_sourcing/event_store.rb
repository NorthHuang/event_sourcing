...
module EventSourcing
  class EventStore
    ...
    def store_events(events, entity_or_nil = nil)
      events.each { |event| event_record_class.save_event(event) }
    ...
    def load_events(
      ...
      records = []
      query = event_record_class.order(version: :asc)
    ...
    def load_multi_aggregate_events(
      ...
      arel_table = event_record_class.arel_table

      aggregate_queries =
        aggregate_selectors.filter_map do |aggregate_selector|
          aggregate_id, from_version =
            aggregate_selector.values_at(:aggregate_id, :from_version)

          next if aggregate_id.blank?
      ...
      return [] if aggregate_queries.empty?

      query =
        query.where(
          arel_table.grouping(
            aggregate_queries.reduce { |s, x| Arel::Nodes::Or.new(s, x) }
          )
        )
    ...
    def event_record_class
      klass = @configuration.event_record_class
      return klass if klass

      raise EventSourcing::ConfigurationError,
            "config.event_record_class must be set"
    end
  end
end
