...
module EventSourcing
  class AggregateRoot
    ...
    class << self
      ...
      def aggregate_type
        parts = name.split('::')
        aggregates_namespace = parts.index('Aggregates')
        parts = parts[(aggregates_namespace + 1)..] if aggregates_namespace
    ...
    MARSHAL_INGORE_VARIABLES = MARSHAL_IGNORE_VARIABLES

    def marshal_dump
      variables = instance_variables - MARSHAL_IGNORE_VARIABLES
      Hash[variables.map { |name| [name, instance_variable_get(name)] }]
    end

    def marshal_load(data)
      data.each { |k, v| instance_variable_set(k, v) }
      @uncommitted_events ||= @uncommited_events || []
      @domains ||= {}
      @applied_events ||= []
    end
